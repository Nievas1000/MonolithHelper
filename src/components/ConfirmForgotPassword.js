...
const ConfirmForgotPassword = ({setUiState, user, handleChange}) =>{
    
    async function forgotPasswordSubmit(e) {
        e.preventDefault()
        try {
            let data = await Auth.forgotPasswordSubmit(user.email, user.authCode, user.password)
            console.log(data);
        } catch (error) {
            console.log(error);
        }
    ...
    return(
        <div>
            <h5 className='mb-3'>Enter the code you recived and set a new password</h5>
            <Form>
                <Form.Group className="mb-3">
                    <Form.Label>Username or Email Address</Form.Label>
                    <Form.Control name='email' type="text" value={user.email} placeholder="Enter email" onChange={handleChange}/>
                </Form.Group>
